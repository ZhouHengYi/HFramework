...
namespace H.Core.Utility
{
    ...
    public class ImageResizingManagerBP
    {
        ...
        /// <summary>
        /// 根据所给的宽、高得到相应比例的水印（带商家水印路径）
        /// </summary>
        /// <param name="imageWidth"></param>
        /// <param name="imageHeight"></param>
        /// <returns></returns>
        private static Bitmap GetWatermark(int imageWidth, int imageHeight, string strVendorWaterPath)
        {
            Bitmap s_imagVendorWatermark = new Bitmap(System.Web.Hosting.HostingEnvironment.ApplicationPhysicalPath + "Item/Resources/Images/" + strVendorWaterPath);
            if (!s_dictWaterMarks.ContainsKey(imageWidth))
            {
                s_imagVendorWatermark.MakeTransparent(Color.Black);
                var precent = GetWatermarkPercent(imageWidth, imageHeight);
                int resizedWidth = (int)(s_imagVendorWatermark.Width * precent);
                int resizedHeight = (int)(s_imagVendorWatermark.Height * precent);

                var result = s_imagVendorWatermark.GetThumbnailImage(resizedWidth, resizedHeight, () => false, IntPtr.Zero);
        ...
        /// <param name="strVendorWaterPath"></param>
        /// <returns></returns>
        public static Bitmap InsertWatermark(Image imgPhoto, string strVendorWaterPath)
        {
            int phWidth = imgPhoto.Width;
            int phHeight = imgPhoto.Height;
            Graphics grPhoto = null;
            ...
            try
            {
                ...
                //create a image object containing the watermark
                Image imgWatermark = GetWatermark(imgPhoto.Width, imgPhoto.Height, strVendorWaterPath);
                int wmWidth = imgWatermark.Width;
                int wmHeight = imgWatermark.Height;
