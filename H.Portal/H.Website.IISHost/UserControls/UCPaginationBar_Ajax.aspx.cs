...
{
    public partial class UCPaginationBar_Ajax1 : System.Web.UI.Page
    {
        #region [ fields ]
        ...
        {
            if (!string.IsNullOrEmpty(Request["PageSize"]) && !string.IsNullOrEmpty(Request["RecordCount"])
                && !string.IsNullOrEmpty(Request["CurrentPageNumber"]))
            {
                this.PageSize = Convert.ToInt32(Request["PageSize"]);
                this.RecordCount = Convert.ToInt32(Request["RecordCount"]);
                this.CurrentPageNumber = Convert.ToInt32(Request["CurrentPageNumber"]);
            }
            ...
            if (ltPrev.Text.IndexOf("span") <= 0)
            {
                ltPrev.Text += "<span>";
            }

            if (ltNext.Text.IndexOf("span") <= 0)
            {
                ltNext.Text = "</span>" + ltNext.Text;
            }
        ...
        {
            return "javaScript:bindHelper.setPage(" + pageNumber + ");";
        ...
        private void SetPreviousPageLink()
        {
            if (this.CurrentPageNumber - 1 <= 0)
            {
                if (this.simpleStyle == true)
                {
                    ltPrev.Text = this.BuildPageElement("上一页", "javaScript:bindHelper.prevPage();", "previous fg-button ui-button ui-state-default ui-state-default");
                }
                else
                {
                    ltPrev.Text = this.BuildPageElement("上一页", "javaScript:bindHelper.prevPage();", "previous fg-button ui-button ui-state-default ui-state-disabled");
                }
            }
            else
            {
                ...
                {
                    ltPrev.Text = this.BuildPageElement("上一页", "javaScript:bindHelper.prevPage();", "previous fg-button ui-button ui-state-default");
                }
            }
        }
        ...
        private void SetNextPageLink()
        {
            if ((this.CurrentPageNumber + 1) > this.PageCount)
            {
                if (this.simpleStyle == true)
                {
                    ltNext.Text = this.BuildPageElement("下一页", "javaScript:bindHelper.nextPage();", "next fg-button ui-button ui-state-default");
                }
                else
                {
                    ltNext.Text = this.BuildPageElement("下一页", "javaScript:bindHelper.nextPage();", "first ui-corner-tl ui-corner-bl fg-button ui-button ui-state-default ui-state-disabled");
                }
            }
            else
            {
            ...
            ltEndPage.Text = this.BuildPageElement(this.PageCount.ToString(), this.BuildPaginationUrl(this.PageCount), "next fg-button ui-button ui-state-default");
        }
        ...
        private void SetEndPageLink()
        {
            if ((this.CurrentPageNumber + 1) > this.PageCount)
            {
                if (this.simpleStyle == true)
                {
                    ltEndPage.Text = this.BuildPageElement("尾页", "javaScript:bindHelper.lastPage();", "btn_next nextDisable inblock ml10");
                }
                else
                {
                    ltEndPage.Text = this.BuildPageElement("尾页", "javaScript:bindHelper.lastPage();", "first ui-corner-tl ui-corner-bl fg-button ui-button ui-state-default ui-state-disabled");
                }
            }
            else
            {
                if (this.simpleStyle == true)
                {
                    ltEndPage.Text = this.BuildPageElement("尾页", "javaScript:bindHelper.lastPage();", "btn_next  inblock ml10");
                }
                else
                {
                    ltEndPage.Text = this.BuildPageElement("尾页", "javaScript:bindHelper.lastPage();", "next fg-button ui-button ui-state-default");
                }
            }
        }
        ...
        private void SetBeginPageLink()
        {
            if (this.CurrentPageNumber - 1 <= 0)
            {
                if (this.simpleStyle == true)
                {
                    ltBeginPage.Text = this.BuildPageElement("首页", "javaScript:bindHelper.firstPage();", "first ui-corner-tl ui-corner-bl fg-button ui-button ui-state-default");
                }
                else
                {
                    ltBeginPage.Text = this.BuildPageElement("首页", "javaScript:bindHelper.firstPage();", "first ui-corner-tl ui-corner-bl fg-button ui-button ui-state-default ui-state-disabled");
                }
            }
            else
            {
                ...
                {
                    ltBeginPage.Text = this.BuildPageElement("首页", "javaScript:bindHelper.firstPage();", "first ui-corner-tl ui-corner-bl fg-button ui-button ui-state-default ui-state-disable");
                }
            }
        }
