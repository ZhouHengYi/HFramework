<%@ Page Title="" Language="C#" MasterPageFile="~/Master/Master.Master" AutoEventWireup="true" CodeBehind="Management.aspx.cs" Inherits="H.Website.IISHost.Pages.Management.Management" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="cph_Content" runat="server">
    <div id="content-header">
        <div id="breadcrumb"><a href="#" title="Go to Home" class="tip-bottom"><i class="icon-home"></i>Home</a> <a href="#" class="current">管理制度</a></div>
                            ...
                            <table class="table table-bordered data-table dataTable" id="tbl_List">
                                ...
                                <tr class="odd gradeX">
                                    ...
                                    <td bind="$.SysNo" width="5%" style="text-align: center;">
                                        <a href="javaScript:mgmt.maintain($.Value)">查看</a>
                                    </td>
                                </tr>
                            </table>
                            <div class="fg-toolbar ui-toolbar ui-widget-header ui-corner-bl ui-corner-br ui-helper-clearfix">
                                <div style="float: left; margin-top: 5px;">
                                    <button class="btn btn-info btn-large" style="font-size: 13.5px; padding: 5px 19px;" onclick="mgmt.add();">新增</button>
                                    <button class="btn btn-danger btn-large" style="font-size: 13.5px; padding: 5px 19px;" onclick="mgmt.deleteSel();">删除</button>
                                </div>
...
<asp:Content ID="Content3" ContentPlaceHolderID="cph_Bottom" runat="server">
    ...
    <script type="text/javascript">
        $(function () {
            ...
            bindHelper.AjaxProObject = Portal.Management;
            bindHelper.init();
        });

        var mgmt = {
            maintain: function (sysNo) {
                $.showWindow('管理制度信息维护', '<%=BuildUrl(H.Website.Facade.PageAlias.ManagementMaintain)%>?sysNo=' + sysNo + '&timer=' + new Date().getDate(), 700, 565);
            },
            add:function(){
                $.showWindow('添加管理制度', '<%=BuildUrl(H.Website.Facade.PageAlias.ManagementMaintain)%>?timer=' + new Date().getDate(), 700, 565);
            },
            deleteSel: function () {
                var ids = bindHelper.getSelectElementSysNo();
                if (ids.length > 0) {
                    $.confirm("确定删除已选中数据吗？<br>系统编号为：" + ids, function () {
                        var result = Portal.Management.Delete(ids);
                        if (result.error) {
